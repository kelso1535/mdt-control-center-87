  ...
  return <div className="fade-in">
      ...
      <div className="mb-3 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <select className="bg-black/50 border border-muted rounded-md px-3 py-1 text-foreground text-sm" value={reportType} onChange={e => setReportType(e.target.value as ReportType)}>
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        <div className="bg-card border border-border p-4 rounded-md">
          {reportType === 'Serial# KALOF' && <div className="mb-4">
              <label className="text-[hsl(var(--police-blue))] block mb-2">Serial Number:</label>
              <Input placeholder="Enter the serial number..." className="bg-black/50 border-border text-white" value={serialNumber} onChange={e => setSerialNumber(e.target.value)} />
            </div>}
          
          <h3 className="text-[hsl(var(--police-blue))] mb-2">Section 1 - {reportType}</h3>
          <Textarea placeholder="Enter details for Section 1..." className="h-32 bg-black/50 border-border text-white mb-4" value={section1Text} onChange={e => setSection1Text(e.target.value)} />
          
          <h3 className="text-[hsl(var(--police-blue))] mb-2">Section 2 - Report Details</h3>
          <Textarea placeholder="Enter detailed information for Section 2..." className="h-48 bg-black/50 border-border text-white" value={section2Text} onChange={e => setSection2Text(e.target.value)} />
          
          <div className="mt-4">
            <Button className="bg-[hsl(var(--police-blue))] hover:bg-[hsl(var(--police-blue))]/80 text-white" onClick={handleAddReport}>
              Submit Report
            </Button>
          </div>
        </div>
